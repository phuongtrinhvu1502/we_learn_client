...
const TableOpportunity = ({ listCourse, filterDropdownVisible, searchText, changeInputSearch, onFilterDropdownVisibleChange, onInputChange, pagination, onDelete, onRemove,
    ...
    const columns = [
        ...
        {
            title: 'Tiêu đề bài học', dataIndex: "course_title",
            ...
            render: (text, record) =>
                <Link to={'/system-content/post-course/' + record.course_id}
                    className="nav-link" ><span>{text}</span></Link>

        },
        {
            title: 'Ngày tạo', dataIndex: "created_date", width: '20%',
        },
        {
            title: 'Người tạo', dataIndex: "full_name", width: '20%',
        },
        {
        ...
        <div>
            {lstPermission.indexOf("TOPIC-CONTENT-2") > -1 &&
                <Button type="primary" size='large' className="margin-bottom-5">
                    <Link to={'/system-content/post-course/'}
